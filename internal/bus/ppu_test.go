...
func TestBus_OAMDMA_StepwiseAndBlocking(t *testing.T) {
    ...
    for i := 0; i < 0xA0; i++ { b.Write(0xC000+uint16(i), byte(i)) }
    // Start DMA from 0xC000
    b.Write(0xFF46, 0xC0)
    // During DMA, OAM reads should be blocked and return 0xFF; writes ignored
    if got := b.Read(0xFE00); got != 0xFF { t.Fatalf("OAM read during DMA got %02X want FF", got) }
    b.Write(0xFE00, 0xEE) // should be ignored
    // After 80 cycles, still in progress
    tick(b, 80)
    if got := b.Read(0xFE10); got != 0xFF { t.Fatalf("mid-DMA OAM read got %02X want FF", got) }
    ...
    b.Write(0xFE00, 0x99)
    if got := b.Read(0xFE00); got != 0x99 { t.Fatalf("OAM write post-DMA failed: got %02X", got) }
}
...
func TestPPU_ModeSequenceVisibleLine(t *testing.T) {
    ...
    if mode := b.Read(0xFF41) & 0x03; mode != 2 { t.Fatalf("mode at start got %d want 2", mode) }
    // After 80 dots -> enter mode 3
    tick(b, 80)
    if mode := b.Read(0xFF41) & 0x03; mode != 3 { t.Fatalf("mode at dot80 got %d want 3", mode) }
    // After 172 more -> enter mode 0
    tick(b, 172)
    if mode := b.Read(0xFF41) & 0x03; mode != 0 { t.Fatalf("mode at dot252 got %d want 0", mode) }
    // Finish line to next line start -> mode 2 and LY=1
    tick(b, 456-252)
    if ly := b.Read(0xFF44); ly != 1 { t.Fatalf("LY after 1 line got %d want 1", ly) }
    if mode := b.Read(0xFF41) & 0x03; mode != 2 { t.Fatalf("mode at new line got %d want 2", mode) }
}
...
func TestPPU_VBlankDurationAndIF(t *testing.T) {
    ...
    // Run 144 lines
    tick(b, 144*456)
    if ly := b.Read(0xFF44); ly != 144 { t.Fatalf("LY at vblank start got %d want 144", ly) }
    if mode := b.Read(0xFF41) & 0x03; mode != 1 { t.Fatalf("mode at vblank start got %d want 1", mode) }
    // VBlank IF must be set
    if (b.Read(0xFF0F) & 0x01) == 0 { t.Fatalf("VBlank IF not set on entering vblank") }
    // VBlank lasts 10 lines (144..153), then wraps to 0
    tick(b, 10*456)
    if ly := b.Read(0xFF44); ly != 0 { t.Fatalf("LY after vblank wrap got %d want 0", ly) }
}
...
func TestPPU_WriteLYResetsLineAndMode(t *testing.T) {
    ...
    tick(b, 252)
    if mode := b.Read(0xFF41) & 0x03; mode != 0 { t.Fatalf("pre-reset mode got %d want 0", mode) }
    b.Write(0xFF44, 0x99) // any value resets LY and dot
    if ly := b.Read(0xFF44); ly != 0 { t.Fatalf("LY not reset to 0: %d", ly) }
    if mode := b.Read(0xFF41) & 0x03; mode != 2 { t.Fatalf("mode after LY reset got %d want 2", mode) }
}
...
func TestPPU_STAT_VBlankInterruptEnable(t *testing.T) {
    ...
    if (b.Read(0xFF0F) & 0x01) == 0 { t.Fatalf("VBlank IF not set") }
    if (b.Read(0xFF0F) & 0x02) != 0 { t.Fatalf("STAT IF set unexpectedly when disabled") }
    // Clear IF and enable STAT VBlank (bit4)
    b.Write(0xFF0F, 0)
    b.Write(0xFF41, 1<<4)
    // Run another full frame to next vblank
    tick(b, 154*456)
    if (b.Read(0xFF0F) & 0x02) == 0 { t.Fatalf("STAT IF not set on VBlank when enabled") }
}
